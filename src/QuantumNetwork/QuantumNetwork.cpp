...
/*vector<float> QuantumData::ChangeToInput(vector<float>& input) {
    ...
    for (int i = 0; i < input.size(); ++i) {
        if(input[i] > 0) {
            output.push_back(input[i] + 0.5);
        }
        else {
            output.push_back(input[i] + 0.0);
        }
    }
    return output;
}*/
...
vector<float> QuantumData::ChangeToInput(vector<vector<float> >& input) {
    ...
    for (int i = 0; i < input.size(); ++i) {
        for (int j = 0; j < input[0].size(); ++j) {
            ...
            if(input[i][j] > 0.0) {
                output.push_back(atan2(input[i][j] + 0.3, 1));
            }
            else {
                output.push_back(atan2(input[i][j] + 0.0, 1));
            }
        }
    }
    return output;
...
void QuantumNeuralNet::CreateNet(){
    if(NumHiddenLayers > 0) {
        VecLayers.push_back(QuantumNeuronLayer(NeuronsPerHiddenLayer, NumInputs, 1));		//输入层
		for(int i = 0; i < NumHiddenLayers - 1; ++i){
			VecLayers.push_back(QuantumNeuronLayer(NeuronsPerHiddenLayer, NeuronsPerHiddenLayer, 2));//隐层
		} 
        VecLayers.push_back(QuantumNeuronLayer(NumOutputs, NeuronsPerHiddenLayer, 3));		//输出层
    }
    else {
        VecLayers.push_back(QuantumNeuronLayer(NumOutputs, NumInputs, 1));
    }
...
void QuantumNeuralNet::InitializeNetwork(){
    for (int i = 0; i < NumLayers; ++i) {								    //权值维数比隐层数多1
...
vector<float> QuantumNeuralNet::Update(vector<float> inputs){
    ...
    for(int j = 0; j < VecLayers[0].NumNeurons; ++j) {
        ...
        outputs.push_back(Hj_temp);//隐层输出
    }
    //计算隐层到输出层
    inputs = outputs;
    outputs.clear();
    for(int m = 0; m < VecLayers[1].NumNeurons; ++m) {
        ...
        outputs.push_back(atan2(VecLayers[1].VecNeurons[m].Activation, 1));//即输出
    }
