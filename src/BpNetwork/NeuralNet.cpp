...
void NeuralNet::CreateNet()
{
    ...
    {
        vecLayers.emplace_back(NeuronLayer(NeuronsPerHiddenLayer,NumInputs));		//输入层
		for (int i = 0; i < NumHiddenLayers - 1; ++i)
        {
			vecLayers.emplace_back(NeuronLayer(NeuronsPerHiddenLayer,NeuronsPerHiddenLayer));//隐层
		}
        vecLayers.emplace_back(NeuronLayer(NumOutputs,NeuronsPerHiddenLayer));		//输出层
    }
    else
    {
        vecLayers.emplace_back(NeuronLayer(NumOutputs,NumInputs));
    }
}
...
vector<float> NeuralNet::Update(vector<float> inputs)
{
    ...
    for (int i = 0;i < NumHiddenLayers + 1; ++i)
    {
        ...
        for (int n = 0; n < vecLayers[i].NumNeurons; ++n)
        {
            ...
            outputs.emplace_back(vecLayers[i].vecNeurons[n].Activation);//即输出
            cWeight = 0;
        }
    }
    return outputs;
